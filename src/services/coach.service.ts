...
import { v4 } from "uuid";
import { CoachesTable } from "../constants/table-names";
...
export const getCoach = async (id: string) => {
    const user: any = (await getItem(CoachesTable, "id", "==", id))[0];
...
export const getCoachByEmail = async (
    ...
    const user: any = (await getItemById(CoachesTable, email)).data();
